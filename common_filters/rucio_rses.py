...
class RucioReplicas(MetaCatFilter):
    ...
    def filter(self, inputs, *params, **ignore):
        ...
        for chunk in inputs[0].chunked(
            1000
        ):  # Rucio can not handle more than 1000 dids at a time
            ...
            replicas = client.list_replicas(
                dids, all_states=False, ignore_availability=False, resolve_archives=False
            )
...
def create_filters(config):
    return {"rucio_replicas": RucioReplicas(config)}
